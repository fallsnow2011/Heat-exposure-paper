...
TYPICAL_DATES = ['2022-07-15']  # representative typical summer day / 鍏稿瀷澶忔棩
...
def process_single_road_shadow(args):
    """Sample one shade raster along a road geometry / 澶勭悊鍗曟潯閬撹矾锛岄噰鏍烽槾褰卞€?""
    idx, geometry_wkt = args
    ...
    try:
        ...
        if len(valid_values) > 0:
            ...
            # Shadow ratio = number of shaded samples divided by the total sample count / 璁＄畻闃村奖姣斾緥 = 闃村奖鍍忕礌鏁?/ 鎬诲儚绱犳暟
...
def process_city(city_name):
    """Process all typical-day shade-attribution steps for a single city / 澶勭悊鍗曚釜鍩庡競"""
    logger.info(f"{'='*60}")
    ...
    for date_str in TYPICAL_DATES:
        ...
        for hour in HOURS:
            ...
            # Allocate building and vegetation shade contributions / 鍒嗛厤寤虹瓚鍜屾琚础鐚?            col_shadow = f'shadow_{hour:02d}'
    ...
    # Save output using a `typical_day` suffix / 淇濆瓨缁撴灉锛屼娇鐢?`typical_day` 鍚庣紑
    suffix = f"_{VERSION}" if VERSION else ""
...
def main():
    ...
    logger.info("="*60)
    logger.info("Road-level shade attribution to building and vegetation sources - typical-day version / 閬撹矾闃村奖寤虹瓚/妞嶈璐＄尞鍒嗛厤璁＄畻锛氬吀鍨嬪鏃ョ増鏈?)
    logger.info("Date / 鏃ユ湡: 2022-07-15")
    logger.info("="*60)
